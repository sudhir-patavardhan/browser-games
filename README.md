...
| [Apogee](apogee/index.html) | A rocket-builder for everyone who ever drew one in a margin. Tap or drag parts from the shelf onto the launch pad — a nose (sleek Aero Cone or heavy Crew Capsule), up to three fuel tanks, an engine (the gentle, fuel-sipping Sparrow or the mighty, thirsty Titan), fins, and strap-on boosters — watching the live mass/thrust/fuel/TWR readout as the stack grows. Then LAUNCH: a synthesized countdown, engine rumble, booster jettison at T+5s, the sky fading from day blue to starfield as milestones flash past (1 km, stratosphere, mesosphere, the 100 km Kármán line), and an apogee readout with stars and a design tip when the rocket falls back. The physics are simple but honest — thrust vs. weight, drag that thins with the atmosphere, finless rockets wobbling their thrust away, a Titan draining a Slim Tank in seconds, and an overloaded Sparrow that never leaves the pad — so climbing from a 3 km hop to space is a real design puzzle. Best apogee saved; your rocket persists between visits. | [kreeda.games/apogee](https://kreeda.games/apogee/) |
...
All nineteen are also playable offline straight from the file system — clone the repo and open any `<game>/index.html` directly, no server required.
