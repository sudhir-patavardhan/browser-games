...
| [Drift](drift/index.html) | A bright, sunny endless drifting game with a **real throttle and brake** — no handbrake. Slides aren't a button: steering yaws the car, the velocity vector chases the heading, and that chase is capped by a lateral grip budget, so carrying too much speed into a corner is what breaks the tail loose. Brake as you turn in and the weight shifts onto the nose, unloading the rear (the deliberate way to start a slide); get back on the power to hold it, and countersteer to catch it. Drive inside the limit and you're safe but score nothing — push past it and the multiplier climbs. Two cameras: **top-down**, or a first-person **driver view** through the windshield, with drifting clouds, bird flocks, the occasional airliner, and a hazed city-outskirts horizon of skyline, hills, pylons and treeline. Yellow tyre smoke, fading skid marks, procedural audio, local best score. **Mobile-first:** in top view, on-screen gas/brake pedals and tap the screen halves to steer; in driver view, drag anywhere to turn the centred wheel while it auto-throttles and a second finger brakes; desktop uses ←/→ or A/D to steer and ↑/↓ or W/S for throttle and brake. | Open `drift/index.html` in any browser |
