...
#   ./drift/verify/run.sh oil        # oil slicks: placement, the grip-budget hit, and the heat payout for using one
#
...
case "$PROBE" in
  ...
  oil)    JS="$HERE/oil.js";    DIV="RESULTS" ;;
  *) echo "unknown probe '$PROBE' (want: assert | controls | contracts | garage | nearmiss | daily | ghost | report | weather | horde | badges | waves | plane | music | land | pause | restnav | oil)" >&2; exit 2 ;;
esac
